...
// Aggregate Root: Haupt-Entity für Bestellungen
public class Order {
    private final String id;                // Order-ID
    private final String customerId;        // Kunde
    private final String restaurantId;      // Restaurant
    private final List<OrderItem> items;    // Positionen
    private final Address deliveryAddress;  // Lieferadresse
    private OrderStatus status;             // Bestellstatus
    private boolean isPaid;                 // Bezahlt?
    private LocalDateTime createdAt;        // Zeitstempel
    private List<DomainEvent> domainEvents; // Domain Events
    ...
    public Order(String id, String customerId, String restaurantId, Address deliveryAddress) {
        ...
        // Event beim Anlegen der Order erzeugen
        this.domainEvents.add(new OrderPlacedEvent(id, restaurantId));
    }

    // Items nur im Status CREATED hinzufügen
    ...
    // In Vorbereitung (nur wenn bezahlt & bestätigt)
    ...
    // Bestellung stornieren (nur solange nicht ausgeliefert)
    public void cancel() {
